Lilith Core version *0.15.1* is now available from:

  <https://download.lilith.org/lilith-0.15.1/>
...
A number of changes to the way Lilith Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 litis per kB (0.00001 LIL). This is to prepare for dropping the minimum transaction fee to 0.1 litis per kB (0.0001 LIL) in the next release.
